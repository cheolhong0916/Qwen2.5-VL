...
model_type = sys.argv[1] # qwen or molmo

if len(sys.argv) > 2:
    model_name = sys.argv[2]# image_prefix_sat_prism

################## QWEN 2.5-VL ##################
if model_type == "qwen":
    # ckpt_path = "Qwen/Qwen2.5-VL-7B-Instruct"
    ckpt_path =f"/data/shared/Qwen/Qwen2.5-VL/qwen-vl-finetune/output/{model_name}"
...
################# MOLMO #################
    ...
    processor = AutoProcessor.from_pretrained("allenai/Molmo-7B-D-0924", torch_dtype="auto", device_map="auto", trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained("allenai/Molmo-7B-D-0924", torch_dtype="auto", device_map="auto", trust_remote_code=True)
...
for i, data in enumerate(prism_test_set):
    ...
    # MOLMO
        ...
        inputs = processor.process(images=data['image'], text=data["prompt"], return_tensors="pt")
    ...
    pred_point = parse_point(output_text)
